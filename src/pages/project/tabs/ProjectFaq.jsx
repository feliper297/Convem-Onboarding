...
function ProjectFaq({ project, pushToast }) {
  const initItems = () => {
    let id = 1;
    return Object.entries(project.faq).flatMap(([cat, qs]) => qs.map((q) => ({ ...q, id: id++, cat })));
  };
  ...
  const save = () => {
    ...
    if (modal.mode === "add") { setItems((prev) => [...prev, { ...form, id: Date.now() }]); pushToast("Pergunta adicionada!", "success"); }
    else { setItems((prev) => prev.map((i) => i.id === modal.item.id ? { ...i, ...form } : i)); pushToast("Pergunta atualizada!", "success"); }
    setModal(null);
  };
  const confirmDelete = () => { setItems((prev) => prev.filter((i) => i.id !== delTarget.id)); pushToast("Pergunta removida.", "default"); setDelTarget(null); };
    ...
    <div className="w-full">
      ...
      {grouped.map(({ cat, items: catItems }) => (
        <div key={cat} className="mb-6">
          <h3 className="text-[12.5px] font-semibold uppercase tracking-wide mb-2.5" style={{ color: "#9AA2B1" }}>{cat}</h3>
          <div className="flex flex-col gap-2">
            {catItems.map((it) => {
              const open = openIdx === it.id;
              return (
                <div key={it.id} className="rounded-xl overflow-hidden" style={{ background: "#fff", border: "1px solid #E4E7EC" }}>
                  <div className="flex items-center justify-between px-4 py-3">
                    <button onClick={() => setOpenIdx(open ? null : it.id)} className="flex items-center gap-2 text-left flex-1">
                      <span className="text-[13px] font-semibold" style={{ color: "#14171F" }}>{it.q}</span>
                      <ChevronDown size={15} className={`transition-transform shrink-0 ml-1 ${open ? "rotate-180" : ""}`} color="#9AA2B1" />
                    </button>
                    <RowActions project={project} onEdit={() => openEdit(it)} onDelete={() => setDelTarget(it)} />
                  </div>
                  {open && <div className="px-4 pb-3.5 text-[12.5px]" style={{ color: "#5B6472" }}>{it.a}</div>}
                </div>
              );
            })}
          </div>
