...
function ProjectPessoas({ project, pushToast }) {
  const [items, setItems] = useState(() => project.pessoas.map((p, i) => ({ ...p, id: i + 1 })));
  ...
  const save = () => {
    ...
    if (modal.mode === "add") {
      ...
      setItems((prev) => prev.map((p) => p.id === modal.item.id ? { ...p, ...form } : p));
      pushToast("Membro atualizado!", "success");
    }
    setModal(null);
  };
  const confirmDelete = () => { setItems((prev) => prev.filter((p) => p.id !== delTarget.id)); pushToast("Membro removido.", "default"); setDelTarget(null); };
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((p) => (
          <div key={p.id} className="rounded-2xl p-4 flex flex-col items-center text-center gap-2 relative" style={{ background: "#fff", border: "1px solid #E4E7EC" }}>
            ...
            <div><p className="text-[13.5px] font-semibold" style={{ color: "#14171F" }}>{p.name}</p><p className="text-[12px]" style={{ color: "#5B6472" }}>{p.role} · {p.area}</p></div>
            <span className="text-[11px] px-2 py-1 rounded-full" style={{ background: "#F7F8FA", color: "#5B6472" }}>{p.specialty}</span>
